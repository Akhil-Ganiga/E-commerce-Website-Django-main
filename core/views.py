...
from .models import (Item, Cart, OrderItem, Address, Payment, 
...
# class cashondelivery(LoginRequiredMixin, View):
#      def get(self, *args, **kwargs):
#          order = Cart.objects.get(user=self.request.user, ordered=False)
...
#          status_url = self.request.build_absolute_uri(reverse('core:cashondelivery'))
#          mypayment = SSLSession(sslc_is_sandbox=True, sslc_store_id=settings.SSLC_STORE_ID,
...
#             return redirect('core:complete_payment', tran_id=payment_data['tran_id'], payment_type="S")
#         elif status == 'FAILED':
